var FormWebServiceController = function ($scope, $http, $stateParams, $state) {
    $scope.typeMessage = 0;
    $scope.message = "";
    $scope.request = {};
    ...
    function loadInfo() {
        ...
        $http.get('Configuration/getWebService?id=' + $stateParams.id, data, config).success(function (resp) {
            if (resp.type !== 'danger') {
                $scope.request = resp;
            } else {
...
FormWebServiceController.$inject = ['$scope', '$http', '$stateParams', '$state'];
